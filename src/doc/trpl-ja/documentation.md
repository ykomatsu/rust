...
このコードは[このような][c-new]見た目のドキュメントを生成します。
...
このことは列挙型のようなもののドキュメントを書くときに重要です。
...
もしあなたの関数にこのような、パニックによって検出されたり強制されたりするような些細ではない取決めがあるのであれば、ドキュメントを書くことは非常に重要です。
...
もしあなたの関数が`unsafe`であれば、あなたは呼出元が維持するためにどの不正について責任を持つべきなのかを説明すべきです。
...
あなたは、テスト可能性を維持しながら、長い例を詳細に説明するためにこのテクニックを使うことができます。
例えば、このコードです。
...
例の全体を繰り返すことで、あなたは、説明に関係する部分だけを見せつつ、例がちゃんとコンパイルされることを保証することができます。
...
これは極めて汎用的なので、必要になることはほとんどありません。
...
`should_panic`は、そのコードが実際にテストとして成功すべきなのではなく、正しくコンパイルされるべきだということを`rustdoc`に教えます。
...
あなたが`//!`を頻繁に見る場所見る場所はここです。モジュールドキュメントです。
...
これは、複数の異なったオプション、つまりロゴ、ファビコン、ルートのURLをセットします。
