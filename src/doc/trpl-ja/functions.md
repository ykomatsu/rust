...
前に私たちが言ったように、`fn`は「これは関数です」ということを示します。この関数には引数がないので、名前と括弧が続きます。そして、その本文を表す波括弧が続きます。
これが`foo`という名前の関数です。
...
これはよく考えられた設計上の決断です。
プログラムによる完全な推論は可能であり、Haskellのようにそれを行っている言語では、しばしばあなたの型を明示的にドキュメント化することがベストプラクティスであるとして提案されます。
私たちは関数の本文での推論を認める一方で、関数に型の宣言を強制することが完全な推論と推論なしとの間のすばらしいスイートスポットであるということで意見が一致したのです。
