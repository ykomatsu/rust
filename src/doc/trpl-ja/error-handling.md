...
言い換えると、コンパイラーは私たちに拡張子が存在しない可能性に対処することを強制するということです。
...
一旦パースが終われば、私たちはプログラムの引数をRustの構造体にデコードすることができます。
...
一旦それが終われば、私たちは私たちの引数のフラグ、この場合は単純なヘルプメッセージのフラグをセットアップします。
一旦私たちがセットアップされた引数のフラグを持てば、私たちは引数のベクター（インデックス0はプログラム名なのでインデックス1から始まります）をパースするために`Options.parse`を使います。
もしこれが成功したならば、私たちはパースされたオブジェクトにマッチを割り当て、成功しなければ、パニックします。
一旦それを通過すれば、私たちはユーザーがヘルプフラグを渡したかどうかをテストし、もし渡していれば、使い方のメッセージをプリントします。
...
一旦私たちがそれを持てば、`city`引数を得て`search`を実行することは単純なことです。
...
一旦私たちがそれを終えれば、Getoptsが残りを実行します。
